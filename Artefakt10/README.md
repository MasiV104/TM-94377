...
* **Wniosek:** Obrazów Docker używamy, żeby mieć przygotowane całe środowisko do importu bez dodatkowej konfiguracji.
...
* **Wniosek:** Analiza statyczna APK daje testerowi możliwość wychwycenia błędów jakościowych i bezpieczeństwa bez uruchamiania aplikacji, tylko na podstawie jej kodu i zasobów.

### 🔹 BLOK 3-4: Fundamenty Skryptowania (Python for QA)
Budowa logiki testowej w języku Python.
* **Co zrobiono:** Uczyłem się przede wszystkim podstawowych struktur danych Pythona i pisania prostych funkcji, które potem można wykorzystać w automatyzacji testów.
...
* **Co zrobiono:** W projekcie używałem głównie selektorów ID do stabilnego lokalizowania elementów oraz XPath, gdy brakowało unikalnych identyfikatorów lub trzeba było odwołać się do struktury widoków. Na urządzeniu symulowałem typowe akcje użytkownika: tapnięcia, wpisywanie tekstu, przewijanie ekranu oraz nawigację między ekranami z asercjami, że wyświetlane są właściwe widoki.
